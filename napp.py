...
try:
    ...
    def format_currency(amt):
        return f"₹{float(amt):,.2f}" if amt else "₹0.00"
...
def admin_dashboard():
    ...
    try:
        if not MODULES_LOADED:
            ...
            todays_orders = [
                {'order_id': 101, 'user_name': 'Demo User', 'total_amount': 450.00, 'status': 'pending', 'item_count': 2, 'order_date': datetime.now()},
                {'order_id': 102, 'user_name': 'Test Customer', 'total_amount': 380.00, 'status': 'processing', 'item_count': 3, 'order_date': datetime.now()}
            ]
            stats = {'totals': {'total_orders': 2, 'total_revenue': 830.00, 'avg_order_value': 415.00}}
            
            # Create simple chart data WITHOUT functions
            chart_data_dict = {
                'timeline': {
                    'labels': ['12 PM', '1 PM', '2 PM', '3 PM'],
                    'orders': [5, 8, 3, 6],
                    'revenue': [2500, 4000, 1500, 3000]
                },
                'items': {
                    'labels': ['Pizza', 'Burger', 'Pasta'],
                    'quantities': [25, 18, 12],
                    'revenues': [6250, 3240, 2160]
                },
                'status': {
            ...
            # Prepare chart data - Get raw data first
            raw_data = prepare_chart_data(
                stats.get('orders_timeline', []),
                stats.get('top_items', []),
                stats.get('status_distribution', [])
            )
            
            # Convert to simple dict (NO FUNCTIONS)
            chart_data_dict = {
                'timeline': {
                    'labels': raw_data.get('timeline', {}).get('labels', []),
                    'orders': raw_data.get('timeline', {}).get('orders', []),
                    'revenue': raw_data.get('timeline', {}).get('revenue', [])
                },
                'items': {
                    'labels': raw_data.get('items', {}).get('labels', []),
                    'quantities': raw_data.get('items', {}).get('quantities', []),
                    'revenues': raw_data.get('items', {}).get('revenues', [])
                },
                'status': {
                    'labels': raw_data.get('status', {}).get('labels', []),
                    'values': raw_data.get('status', {}).get('values', []),
                    'colors': raw_data.get('status', {}).get('colors', [])
                }
            }
        ...
        return render_template('dashboard.html',
                             todays_orders=todays_orders or [],
                             today_stats=today_stats,
                             pending_stats=pending_stats,
                             customer_stats=customer_stats,
                             chart_data=chart_data_dict,  # ✅ SIMPLE DICT
                             stats=stats,
...
def admin_orders():
    ...
    try:
        if not MODULES_LOADED:
            ...
            orders = [
                {'order_id': 101, 'user_name': 'Demo User', 'user_phone': '+91 9876543210', 
                 'user_email': 'demo@email.com', 'total_amount': 450.00, 'status': 'pending', 
                 'item_count': 2, 'order_date': datetime.now(), 'payment_mode': 'COD',
                 'delivery_location': 'Test Address'},
                {'order_id': 102, 'user_name': 'Test Customer', 'user_phone': '+91 9876543211',
                 'user_email': 'test@email.com', 'total_amount': 380.00, 'status': 'processing', 
                 'item_count': 3, 'order_date': datetime.now(), 'payment_mode': 'Online',
        ...
        flash(f'Error loading orders: {str(e)[:100]}', 'error')
        return render_template('orders.html',
                             orders=[],
                             page=1,
                             total_pages=0,
...
def admin_customers():
    ...
    try:
        if not MODULES_LOADED:
            ...
            customers = [
                {'id': 1, 'full_name': 'Demo User', 'phone': '+91 9876543210', 
                 ...
                 'created_at': datetime.now(), 'is_active': True, 'profile_pic': None}
            ]
            total = 1
            total_pages = 1
        else:
        ...
        flash(f'Error loading customers: {str(e)[:100]}', 'error')
        return render_template('customers.html',
                             customers=[],
                             page=1,
                             total_pages=0,
...
def admin_statistics():
    ...
    try:
        ...
        flash(f'Error loading statistics: {str(e)[:100]}', 'error')
        return render_template('statistics.html',
                             stats={},
                             chart_data={
                                 'timeline': {'labels': [], 'orders': [], 'revenue': []},
...
def admin_items():
    ...
    try:
            ...
            if item_type == 'services':
                items = [
                    {'id': 1, 'name': 'Home Cleaning', 'price': 500.00, 'final_price': 450.00, 
                     ...
                     'times_ordered': 5, 'total_quantity': 8, 'status': 'active', 'description': 'Professional home cleaning'},
                    {'id': 2, 'name': 'Car Wash', 'price': 300.00, 'final_price': 270.00,
                     'discount': 30.00, 'category': 'Automotive', 'photo': None,
                     'times_ordered': 3, 'total_quantity': 5, 'status': 'active', 'description': 'Complete car wash'}
                ]
            else:
                items = [
                    {'id': 1, 'name': 'Pizza', 'price': 250.00, 'final_price': 225.00,
                     'discount': 25.00, 'category': 'Italian', 'photo': None,
                     'times_ordered': 25, 'total_quantity': 30, 'status': 'active', 'description': 'Delicious cheese pizza'},
                    {'id': 2, 'name': 'Burger', 'price': 120.00, 'final_price': 108.00,
                     'discount': 12.00, 'category': 'Fast Food', 'photo': None,
                     'times_ordered': 18, 'total_quantity': 22, 'status': 'active', 'description': 'Juicy burger'}
                ]
        else:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
        ...
        flash(f'Error loading items: {str(e)[:100]}', 'error')
        return render_template('items.html',
                             items=[],
                             item_type='services',
                             current_search='',
